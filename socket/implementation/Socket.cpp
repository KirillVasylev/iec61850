...
Socket::Socket(int _port, int _TimeOut_Sec, int _TimeOut_Usec) {
	parcel.clear();
	clients.clear();
	port = _port;
	TimeOut_Sec = _TimeOut_Sec;
	TimeOut_USec = _TimeOut_Usec;
...
void Socket::send_message(int socket, int size, const char* message) {
	send(socket, message, size, 0);
}
void Socket::send_message(Transport parcel)
{
	send_message(parcel.socket, parcel.buff.size(), parcel.buff.c_str());
}
Socket::~Socket()
{
	close_socket();
